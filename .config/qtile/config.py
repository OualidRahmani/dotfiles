...
def get_net():
    try:
        return subprocess.check_output([SYSMON, "net"], timeout=2).decode().strip()
    except Exception:
        return "↑ ↓"
...
# Use a list so multi-character names work correctly in future
group_names = list("123456789")

def add_group(qtile):
    try:
        group_name = subprocess.check_output(
        ...
        pass  # User hit ESC or closed Rofi
...
def delete_group(qtile):
    ...
    if len(group.windows) > 0:
        subprocess.Popen([
            'notify-send', 'Workspace Busy',
            'Close all windows before deleting this workspace.'
        ])
        return
...
# Restore any dynamic groups that existed before a restart
# Uses startup_complete so windows are fully mapped first
...
keys.extend([
    ...
    *[Key([mod, "shift"], i.name, lazy.window.togroup(i.name, switch_group=True))   for i in groups],
])
...
floating_layout = layout.Floating(
    float_rules=[
        *layout.Floating.default_float_rules,
        # Run `xprop WM_CLASS` on each popup to confirm exact strings
    ...
    Screen(
        top=bar.Bar(
            [
                ...
                # Network — wired up to sysmon net
                widget.GenPollText(
                    func=get_net,
                    update_interval=1,
                ...
                widget.GenPollText(
                    ...
                    mouse_callbacks={
                        'Button1': lambda: subprocess.Popen([
                            "python3",
                            os.path.expanduser("~/.local/bin/audio_menu")
                        ])
                    },
                ),
                widget.Sep(padding=10, foreground=colors["surface"]),
                ...
                # Power menu — removed QuickExit (no-confirmation kill), use power_menu only
                widget.TextBox(
                    text="⏻",
                    font="JetBrains Mono",
                    fontsize=18,
...
follow_mouse_focus    = True   # consistent with bring_front_click
...
auto_minimize         = True
wmname = "LG3D"  # Tricks Java/AWT apps into rendering correctly on tiling WMs
